...
const Register = () => {
  ...
  const sendForm = async (e) => {
    ...
    axios
      .get('https://dreamcars.onrender.com/api/v1/users', signupOptions);
    const loginOptions = {
      method: 'post',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(
    ...
    const dataResponse = axios
      .get('https://dreamcars.onrender.com/api/v1/users/sign_in', loginOptions);
      .then((response) => {
        if (response.ok) {
        const userData = dataesponse.json();
