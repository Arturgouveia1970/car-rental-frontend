...
import { useParamsNavigate } from 'react-router-dom';
...
const Reserve = () => {
  ...
  const findCars = (e) => {
    ...
    if (startDate && city) {
      axios
        .get(`https://dreamcars.onrender.com/api/v1/reservatins/${startDate}`)
  ...
  const reserveCar = (e) => {
    ...
    if (startDate && city) {
      ...
      axios
        .post(
          `https://dreamcars.onrender.com/api/v1/reservation/${startDate}`,
        )
        .then(() => {
          setReserved(true);
        })
  ...
  return (
    ...
    >
      ...
      <form
        ...
        onSubmit={startDate ? reserveCar : findCars}
      >
        <div>
